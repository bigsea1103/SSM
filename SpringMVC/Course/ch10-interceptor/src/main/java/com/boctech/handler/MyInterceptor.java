...
public class MyInterceptor implements HandlerInterceptor {
     ...
     *      拦截器的MyInterceptorDE preHandle()
     *      ====执行MyController中的doSome方法===
     *      拦截器的MyInceptor的postHandler()
     *      拦截器的MyInceptor的afterCompletion()
     *      false: 请求没用通过拦截器的验证，请求到达拦截器就截止了，请求没有被处理
     *      拦截器的MyInterceptorDE preHandle()
     *
     *      特点：
     ...
     *      反之，可以放行请求，此时控制器方法才能执行
     * */
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
       btime = System.currentTimeMillis();
        System.out.println("拦截器的MyInterceptorDE preHandle()");
     ...
     *  2.能够获取处理器方法的返回值ModelAndView,可以修改ModelAndView中
     *  的数据和视图，可以影响到最后的执行结果
     *  3.主要是对原来的执行结果做二次修正
     * */
    @Override
    public void postHandle(HttpServletRequest request,
        ...
        System.out.println("拦截器的MyInceptor的postHandler()");
       //对原来的doSome执行结果，需要调整
        if(mv != null) {
            //修改数据
    ...
    public void afterCompletion(HttpServletRequest request,
        ...
        System.out.println("拦截器的MyInceptor的afterCompletion()");
